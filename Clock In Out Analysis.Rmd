...
time.summary <- function(time.field) {
  ...
  min.time <- sorted.times[1]
  max.time <- sorted.times[length(sorted.times)]
  
  # Calculate median
  n <- length(sorted.times)
  median.time <- if (n %% 2 == 1) {
    sorted.times[(n + 1) / 2]
  } else {
    (sorted.times[n / 2] + sorted.times[n / 2 + 1]) / 2
  }
  ...
  formatted.summary <- tibble(
    statistic = c("Min", "Median", "Max", "Mode", "Mean"),
...
```{r}
# Clock in summary
datatable(time.summary(data$clock.in))
```

```{r}
datatable(time.summary(data$clock.out))
```
