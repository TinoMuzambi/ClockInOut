...
We then collapse the data back into the original number of rows to keep with the requirements of tidy data. However, we do this by grouping by the ID and using the max aggregate function on the event columns which means we lose the other columns.
```{r collapse}
...
We remedy this by performing a full join on the original data.
```{r combine}
# Combine dataframes
...
We use `geom_jitter` to jitter overlapping points so it is visible when there are multiple events on one day. We also use transparency to show multiple events.
