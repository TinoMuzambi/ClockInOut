...
# Plot graph
office.time.plot <- ggplot(daily.hours, aes(x = date, y = office.time, colour = event)) +
  geom_point(size = 3, alpha = 0.7,na.rm = T) +
  geom_hline(yintercept = mean.office.time, linetype = "dashed", size = 1) + 
...
office.time.plot + geom_hline(data = event.time, aes(yintercept = mean.hours, colour = event), linetype = "dashed", alpha = 0.7)
```
