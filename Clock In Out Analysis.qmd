...
Finally we can visualise the data. We start with a scatter plot that shows the time spent in office throughout the year. The mean time is also overlayed on the plot.
...
daily.hours <- data.tidy %>%
  dplyr::group_by(date) %>% 
  mutate(office.time = difftime(clock.out, clock.in, units = "hours")) %>% 
  select(c(clock.in, clock.out, office.time, event))
...
# Plot graph
ggplot(daily.hours, aes(x = date, y = office.time, colour = event)) +
  geom_point(size = 3, alpha = 0.7,na.rm = T) +
  geom_hline(yintercept = mean.office.time, linetype = "dashed", alpha = 0.5) + 
